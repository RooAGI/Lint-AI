...
pub enum ClaudeCodeDocumentType {
    Checkpoint,
    Outcome,
    SessionSummary,
}

impl ClaudeCodeDocumentType {
...
impl ClaudeCodeDocument {
    pub fn into_source_document(self) -> Result<SourceDocument> {
        ...
        let mut filters = BTreeMap::from([
            ("integration".to_string(), "claude-code".to_string()),
        ...
        let content = format!("Claude Code {document_type}\n{}", self.content.trim());
        ...
        Ok(SourceDocument {
            ...
            group_id: Some(format!("claude-session:{project_id}:{}", self.session_id)),
...
pub fn project_id(cwd: &Path) -> Result<String> {
    let canonical = cwd.canonicalize().with_context(|| {
        format!(
            "failed to canonicalize Claude working directory {}",
            cwd.display()
        )
    })?;
...
mod tests {
    ...
    fn claude_document_maps_to_session_source_document() {
        ...
        let source = ClaudeCodeDocument {
            event_id: "event-1".to_string(),
            session_id: "session-1".to_string(),
            document_type: ClaudeCodeDocumentType::Outcome,
    ...
    fn event_id_makes_document_id_deterministic() {
        let cwd = std::env::current_dir().unwrap();
        let build = || ClaudeCodeDocument {
            event_id: "same-event".to_string(),
            session_id: "session-1".to_string(),
            document_type: ClaudeCodeDocumentType::Checkpoint,
