...
> **0.2.0 compatibility update:** the deprecated query wrappers described
> below were retained in 0.2.0 to avoid an unnecessary source break. Their
> removal is deferred to a later breaking release.
...
the server and a Tantivy dependency upgrade.

> ## ⚠️ Deprecation warning
>
> Five `MemoryIndex` query methods are **deprecated as of this release** and
> were originally planned for removal in `0.2.0`:
>
...
>
> Migrate before a future breaking release. See [Deprecations](#deprecations) for
...
- **Localhost-only binding.** The server refuses every non-loopback bind
  address. `--allow-unauthenticated` is available for single-user local use.
...
segment routing to unstemmed cost roughly 5pp of fractional Recall@5, and
switching the lexical path to stemmed cost roughly 0.1–0.3pp across fractional
recall, MRR, and NDCG.
...
- `tantivy` `0.22` → `0.25`, which brings in a fixed `crossbeam-epoch`
  ...
  cost roughly 1.7pp any-hit Recall@5 and 1.3pp MRR on LongMemEval-S, while `0.25`
  reproduces `0.1.8` results within noise.
- `pyo3` updated to clear a security audit finding.

## Deprecations

The following `MemoryIndex` methods were deprecated in `0.1.9`. They remain as
compatibility wrappers in 0.2.0, and each carries a `#[deprecated]` note naming
its replacement.
...
fractional Recall@5/10/20, any-hit Recall@5/10/20, MRR, and NDCG@10. Upgrading does not
