...
def draw_frame(lines, footer):
    ...
    d.text((132, 39), "Lint-AI • chronology-only current-state demo", font=SMALL, fill=MUTED)
    ...
    for line in visible:
        ...
        if line.startswith("$"):
            color, f = ACCENT, BOLD
        elif '"semantic_status": "current"' in line or "retry attempts: 2" in line or '"stale_value_in_context": false' in line:
            color = GREEN
        elif "retry attempts: 5" in line and not line.startswith("$"):
            color = MUTED
    ...
    for line in lines:
        if line.startswith("$"):
            for n in range(1, 7):
                ...
                frames.append(draw_frame(shown + [partial + ("▌" if n < 6 else "")], "Real binary • neutral query • no supersedes metadata"))
            shown.append(line)
        else:
            shown.append(line)
            frames.extend([draw_frame(shown, "Real binary • neutral query • no supersedes metadata")] * 3)
